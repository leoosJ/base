package com.team.base.module.controller.login;
...
import com.team.base.module.entity.login.Login;
import com.team.base.module.entity.sys.user.User;
import com.team.base.module.service.login.TokenService;
    ...
    public ResponseResult<User> login(Login login, HttpServletResponse response, HttpServletRequest request) {
        ...
        if (value != null && (Integer)value >= 3) {
            ...
            if (StringUtils.isEmpty(login.getVerifyCode())) {
                result.setCode(LoginStatusEnum.C1002.getStatus());
                result.setMsg(LoginStatusEnum.C1002.getMessage());
                return result;
            } else {
                ...
                if (verification) {
                    ...
                    // 验证码错误
                    result.setCode(LoginStatusEnum.C1001.getStatus());
                    result.setMsg(LoginStatusEnum.C1001.getMessage());
                    return result;
                }
            }
        }
        ...
        // 根据帐号获取用户信息
        User user = this.userService.findUserByUserName(login.getUserName());
        ...
        if (user == null || !login.getPassword().equals(user.getPassword())) {
            if (value == null) {
                ...
                if ((Integer)value == 2) {
                    result.setCode(LoginStatusEnum.C1004.getStatus());
                    result.setMsg(LoginStatusEnum.C1004.getMessage());
                    return result;
                }
            }

            result.setCode(LoginStatusEnum.C1000.getStatus());
            result.setMsg(LoginStatusEnum.C1000.getMessage());
            return result;
        }
